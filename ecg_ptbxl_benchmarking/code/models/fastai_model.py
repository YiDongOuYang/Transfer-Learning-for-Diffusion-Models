...
from resnet1d import resnet1d18,resnet1d34,resnet1d50,resnet1d101,resnet1d152,resnet1d_wang,resnet1d,wrn1d_22
from xresnet1d import xresnet1d18,xresnet1d34,xresnet1d50,xresnet1d101,xresnet1d152,xresnet1d18_deep,xresnet1d34_deep,xresnet1d50_deep,xresnet1d18_deeper,xresnet1d34_deeper,xresnet1d50_deeper
from inception1d import inception1d
from basic_conv1d import fcn,fcn_wang,schirrmeister,sen,basic1d,weight_init
...
# sys.path.append("Diffusion_RL/ecg_ptbxl_benchmarking/code/utils")
from util_utility.utils import evaluate_experiment
...
class fastai_model(ClassificationModel):
    ...
    # that was used for generation/training the guidance network
    def predict_guidance(self, X):
        X = [l.astype(np.float32) for l in X]
    ...
    def get_embedding(self, X):
        # ipdb.set_trace()
        y_dummy = [np.ones(self.num_classes,dtype=np.float32) for _ in range(len(X))]
        learn = self._get_embedding_learner(X,y_dummy,X,y_dummy)

        # learn.load(self.name)

        state_dict = torch.load('Diffusion_RL/ecg_ptbxl_benchmarking/output/exp_ICBEB_from_scatch/models/fastai_xresnet1d50/models/fastai_xresnet1d50.pth')
        filtered_state_dict = {k: v for k, v in state_dict['model'].items() if not k.startswith('8.6.') and not k.startswith('8.8.')}
        learn.model.load_state_dict(filtered_state_dict)

        # with open('./model2.txt', 'w') as file:
        #     file.write(str(filtered_state_dict.keys()))

        preds,targs=learn.get_preds()
        reshaped_tensor = preds.view(len(X), 7, 128)
        embedding = reshaped_tensor.mean(dim=1)
        return embedding

    def get_embedding_for_real_data(self, X):
        # ipdb.set_trace()

        filtered_arrays = [arr for arr in X if arr.shape[0] >= 1000]
        filtered_arrays = [l.astype(np.float32)[:1000] for l in filtered_arrays]

        y_dummy = [np.ones(self.num_classes,dtype=np.float32) for _ in range(len(filtered_arrays))]
        learn = self._get_embedding_learner(filtered_arrays,y_dummy,filtered_arrays,y_dummy)

        # learn.load(self.name)

        state_dict = torch.load('Diffusion_RL/ecg_ptbxl_benchmarking/output/exp_ICBEB_from_scatch/models/fastai_xresnet1d50/models/fastai_xresnet1d50.pth')
        #0509
        # state_dict = torch.load('Diffusion_RL/ecg_ptbxl_benchmarking/output/exp_ICBEB_correct_0427/models/fastai_xresnet1d50/models/fastai_xresnet1d50.pth')
        filtered_state_dict = {k: v for k, v in state_dict['model'].items() if not k.startswith('8.6.') and not k.startswith('8.8.')}
        learn.model.load_state_dict(filtered_state_dict)
    ...
    def _get_learner_model(self, num_classes=None):
        # df_train = pd.DataFrame({"data":range(len(X_train)),"label":y_train})
        # df_valid = pd.DataFrame({"data":range(len(X_val)),"label":y_val})
    ...
    def _get_embedding_learner(self, X_train,y_train,X_val,y_val,num_classes=None):
        df_train = pd.DataFrame({"data":range(len(X_train)),"label":y_train})
        df_valid = pd.DataFrame({"data":range(len(X_val)),"label":y_val})
        ...
        # get embedding
        model = nn.Sequential(*list(model.children())[:-1] + [list(model.children())[-1][:-4]])
        learn = Learner(db,model, loss_func=loss, metrics=metrics,wd=self.wd,path=self.outputfolder)
